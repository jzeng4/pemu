...
//	if(pemu_exec_stats.PEMU_start == 0
//			|| pemu_exec_stats.PEMU_cr3 != PEMU_get_cr3()) //is specified process?
//		return;

	//gen_helper_inst_hook(tcg_const_ptr((void*)print), tcg_const_i32(0));
	switch(num){
		case 1:
			gen_helper_bbl_hook_1(tcg_const_ptr(func_args[0]));
			break;
		case 2:
			gen_helper_bbl_hook_2(tcg_const_ptr(func_args[0]), tcg_const_ptr(func_args[1]));
			break;
		case 3:
			gen_helper_bbl_hook_3(tcg_const_ptr(func_args[0]), tcg_const_ptr(func_args[1]), tcg_const_ptr(func_args[2]));
			break;
		case 4:
			gen_helper_bbl_hook_4(tcg_const_ptr(func_args[0]), tcg_const_ptr(func_args[1]), tcg_const_ptr(func_args[2]), 
					tcg_const_ptr(func_args[3]));
			break;
		case 5:
			gen_helper_bbl_hook_5(tcg_const_ptr(func_args[0]), tcg_const_ptr(func_args[1]), tcg_const_ptr(func_args[2]), 
					tcg_const_ptr(func_args[3]), tcg_const_ptr(func_args[4]));
			break;
		default:
			assert(0);
	}
